...
def normalize_points(points: np.ndarray) -> np.ndarray:
    normalized, _ = normalize_points_with_scale(points)
    return normalized


def normalize_points_with_scale(points: np.ndarray) -> tuple[np.ndarray, float]:
    if len(points) == 0:
        return points, 1.0
...
def build_gaussians_optimized(
    ...
    image_size: int = 0,
    max_views: int = 0,
    cuda_memory_guard_fraction: float = 0.0,
) -> GaussianBuildResult:
    ...
    if backend in {"auto", "gsplat"}:
        try:
            data, initial_loss, final_loss = optimize_gaussians_gsplat(
                ...
                max_views=max_views,
                cuda_memory_guard_fraction=cuda_memory_guard_fraction,
            )
...
def _inverse_softplus_np(value: float) -> float:
...
def optimize_gaussians_gsplat(
    ...
    scales_param = torch.nn.Parameter(
        torch.full((len(points), 3), _inverse_softplus_np(scale0), dtype=torch.float32, device=device)
    )
    ...
    def export_data(note: str) -> dict:
        with torch.no_grad():
            opt_points = means.detach().cpu().numpy()
            norm_points, norm_scale = normalize_points_with_scale(opt_points)
            colors_rgb = torch.sigmoid(color_logits).detach().cpu().numpy()
            raw_radii = torch.nn.functional.softplus(scales_param).mean(dim=1).detach().cpu().numpy()
            radii = np.clip(raw_radii / max(norm_scale, 1e-8), 0.002, 0.035)
    ...
    def memory_guard_triggered() -> bool:
        if cuda_memory_guard_fraction <= 0:
            return False
        free, total = torch.cuda.mem_get_info()
        used_fraction = 1.0 - float(free) / max(float(total), 1.0)
        return used_fraction >= cuda_memory_guard_fraction
    ...
    try:
        ...
        for step in range(iterations):
            if memory_guard_triggered():
                ...
                data = export_data(
                    f"Gaussian parameters partially optimized with gsplat; stopped at iteration {step} by CUDA memory guard."
                )
        ...
        with torch.no_grad():
            final_loss = float(compute_loss().detach().cpu())
        data = export_data("Gaussian parameters optimized with the open-source gsplat CUDA rasterizer.")
    except torch.cuda.OutOfMemoryError:
        torch.cuda.empty_cache()
        data = export_data("Gaussian parameters partially optimized with gsplat; stopped by CUDA out-of-memory guard.")
