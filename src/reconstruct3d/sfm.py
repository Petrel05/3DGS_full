...
def build_tracks_from_pairs(keypoints, pair_matches):
    uf = UnionFind()
    for image_a, image_b, matches in pair_matches:
        for match in matches:
            uf.union((image_a, match.queryIdx), (image_b, match.trainIdx))
...
def build_tracks(keypoints, pair_matches):
    indexed = [(i - 1, i, matches) for i, matches in enumerate(pair_matches, start=1)]
    return build_tracks_from_pairs(keypoints, indexed)
...
def reconstruct_with_known_cameras(
    ...
    pair_matches = []
    pair_inliers = []
    match_window = max(1, int(match_window))
    for image_a in range(len(keypoints)):
        for step in range(1, match_window + 1):
            image_b = image_a + step
            if image_b >= len(keypoints):
                if not loop_closure:
                    continue
                image_b %= len(keypoints)
                if image_b <= image_a:
                    continue
            matches = match_descriptors(descriptors[image_a], descriptors[image_b])
            pair_matches.append((image_a, image_b, matches))
            if step == 1:
                pair_inliers.append(len(matches))
    tracks = build_tracks_from_pairs(keypoints, pair_matches)
